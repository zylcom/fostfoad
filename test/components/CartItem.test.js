...
describe("CartItem.vue Test", () => {
  ...
  it("should pass quantity data same as props in ProductCard component", () => {
    expect(
      wrapper.findComponent("product-card-stub").props().product
    ).toStrictEqual(product);
  });
  ...
  it("should call 'deleteCartItemById' action with cartItemId when button is clicked", async () => {
    await wrapper.get("button").trigger("click");

    expect(wrapper.vm.cartStore.deleteCartItemById).toHaveBeenCalledWith(
      cartItemId
    );
  });
});
